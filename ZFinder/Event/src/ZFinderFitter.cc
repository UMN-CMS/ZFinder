...
//       Data_reco = new RooDataSet("Data_reco","Data_reco",*ZEventArgSet); 
	...
	if (bin0eta>=0 && bin0pt>=0)weight0=EfficiencySF[bin0eta][bin0pt][0];
	if (bin1eta>=0 && bin1pt>=0)weight1=EfficiencySF[bin1eta][bin1pt][0];
      }
      ...
      ZEventArgSet->setRealValue("Pass",1);
      zf_event.is_real_data ? Data_reco->add(*ZEventArgSet) : MC_reco->add(*ZEventArgSet);
